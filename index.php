    ...
    padding:0"   id="contributors" onClick="showModal()">Contributors</button>
			...
			<form id = "ALL" name = "ALL" action = "" method = "POST">
				...
				<div class = "slide-out-div-bottomLeft1" style="height:60%;	" >
					...
					<div class="algoBox2" id="algo4" style="height:6%;" onclick="addRowForAlgo('KMedoids')"><h4 style=" text-align: center;vertical-align: middle;">K-Medoids</h4></div>	
				...
				<div id = "table-scatterPlot">
					...
					<table class = "table1" id ="scatter_tableId" style = "width : 100%">
						...
						<th></th>
							...
							<!--diversity-->
							<th title="Diversity">Div</th>
							<!--relevance-->
							<th title="Relevnecy">Rel</th>
							<!--Radius of Gyration-->
							<th title="Radius of Gyration">G</th>
							<!--Run Time-->
							<th title="RunTime">RT</th>
						</tr>
					</table><br>
					</div>
					</div>
			...
			var rankedResults=[];
				...
				/* Makes direction requests to get path and other attributes like distance time length*/
				...
				function calculateAndDisplayRoute(directionsService, directionsDisplay,locations,waypts,names,a ,isOptimize) {
					...
					/* show path depending on which checkbox is clicked (random walk / highest intensity / shortest path)*/
					function showPath(){
					...
					/*     Display path table dynamically            */
					...
					if(flagForPathTable){
						...
						var br = document.createElement('BR');
							...
							var td = document.createElement('TD');
							td.width = '85';
							var venues=(document.getElementById('noVenues').value != "" && document.getElementById('noVenues').value< document.getElementById('numVenues').value) ? document.getElementById('noVenues').value: document.getElementById('numVenues').value;
							td.appendChild(document.createTextNode(venues));
							tr.appendChild(td);
							tableBody.appendChild(tr);
							table.appendChild(tableBody);
							myTableDiv.appendChild(table);
		...
		<!--path tab -->
				...
				<div class="slide-out-div-topRight ">
					<a class="handleTopRight" href="http://link-for-non-js-users">Content</a><br>
						...
						<br/>
							    ...
							    <button class="buttonsForPath" onClick="resetInPath()">Reset</button>
				...
				<div id="myModal" class="modal">
  				...
  				<!-- Modal content for Contributors -->
				  ...
				  <div class="modal-header"style="height:6%;" >
					...
					<div style="float:left; text-align:center;display:inline; margin-left:10%;"><h3>Back End Server:</h3>
					<p>Xiaoyu Ge</p> 
					</div>
				...
				function uploadFile(){
					...
					if(document.querySelector('input[type=file]').files[0] != null && uploadOnChange()){
				...
				function addAlpha(alpha){
					...
					console.log(rowToChange-1);
					algorithmStructure[rowToChange-1][3]=parseFloat(alpha);
					...
					console.log(algorithmStructure);
				...
				function addLambdSigma(lambda,sigma){
					...
					console.log(rowToChange-1);
					var x=algorithmStructure[rowToChange-1]
					x[4]=parseFloat(lambda);
					x[5]=parseFloat(sigma);
					if(prefDivFlag){
				...
				//check and validate file file uploaded by user
				...
				//This function is called when user clicks on the algorithm box on algorithms tab
				...
				var modal = document.getElementById('myModal');
					...
					/*  Dynamically creates a row on algorithms tab for the clicked algorithm  */
					function addRowForAlgo(name){
						...
						switch(name) {
							...
							case "prefDiv":
								...
								algorithmStructure.push([totalAlgo,name,prefDivNum-1,0.2,0.5,0.2]);
								break;
							case "pPrefDiv":
							models[1]=1;
								pPrefDivNum = pPrefDivNum+1;
								algorithmStructure.push([totalAlgo,name,pPrefDivNum-1,0.2,0.5,0.2]);
								break;
							...
							case "PageRank":
								...
								algorithmStructure.push([totalAlgo,name,pageRankNum-1,0.2,0.5,0.2]);
								
								break;
							case "DisC":
								models[4]=1;
								disCNum = disCNum+1;
								algorithmStructure.push([totalAlgo,name,disCNum-1,0.2,0.5,0.2]);
								break;
							case "KMedoids":
								models[3]=1;
								kmedoidNum = kmedoidNum+1;
								algorithmStructure.push([totalAlgo,name,kmedoidNum-1,0.2,0.5,0.2]);
								break;
							case "Random":
								models[5]=1;
								randomNum = randomNum+1;
								algorithmStructure.push([totalAlgo,name,randomNum-1,0.2,0.5,0.2]);
								break;
							default : custonModels="on";
							
							customModelsNames.push(name);
							algorithmStructure.push([totalAlgo,name,customModelsNames.length-1,0.2,0.5,0.2]);
							break;
						...
						customAlgoModal2.style.display = "none";
						 var myTableDiv = document.getElementById("emptySpace");
							...
							var th = document.createElement('TH');
							th.width = '105';
							th.appendChild(document.createTextNode("Sr."));
							tr.appendChild(th);
							var th = document.createElement('TH');
							th.width = '105';
							...
							var td = document.createElement('TD');
							td.width = '105';
							td.appendChild(document.createTextNode(totalAlgo));
							tr.appendChild(td);
							...
							td.addEventListener("click", (function(){ openCustomizeAlgo("something") ;rowToChange=$(this).parent().index()}));
							tr.appendChild(td);
							var td = document.createElement('TD');
							td.width = '105';
							td.appendChild(document.createTextNode('S'));
							td.addEventListener("click", (function(){ openCustomizeAlgo2("something") ; rowToChange=$(this).parent().index()}));
							tr.appendChild(td);
							var td = document.createElement('TD');
							td.id=enableCounter;
							td.width = '105';
					...
					}// This function creates the input in json format but sends it as a string to the file handler.php usinf ajax call.
					...
					function showloader(){
						...
						for(i=0;i<algorithmStructure.length;i++){
							
								str+='\\"'+algorithmStructure[i][1]+'Alpha'+algorithmStructure[i][2]+'\\":\\"'+algorithmStructure[i][3]+'\\",';
								str+='\\"'+algorithmStructure[i][1]+'Lambda'+algorithmStructure[i][2]+'\\":\\"'+algorithmStructure[i][4]+'\\",';
								str+='\\"'+algorithmStructure[i][1]+'Sigma'+algorithmStructure[i][2]+'\\":\\"'+algorithmStructure[i][5]+'\\",';
							
						}
						if(prefDivNum > 0){     //Counts the number of each algorithm
						...
						var custString="";
						if(custonModels=="on"){
							custString='\\"CustomModelNames\\":\\"'+customModelsNames+'\\",';
						...
						StringToSend+='{\\"UserLocation\\":\\"'+userLocation.lat()+';'+userLocation.lng()+'\\",'+input +'\\"prefDivRouteLength\\":\\"4\\",\\"pPrefDivRouteLength\\":\\"4\\",\\"PageRankRouteLength\\":\\"4\\",\\"DisCRouteLength\\":\\"4\\",\\"KMedoidsRouteLength\\":\\"4\\",\\"RandomRouteLength\\":\\"4\\",\\"CustomModels\\":\\"'+custonModels+'\\",'+custString+str+'}';
				...
				for(var i=0;i<obj.Results.length;i++){
					...
					var tr = document.createElement('TR');
							...
							td.id=enableCounter;
							td.width = '105';
							td.appendChild(document.createTextNode(rg.toFixed(4)));
							tr.appendChild(td);
							var td = document.createElement('TD');
							td.id=enableCounter;
							td.width = '105';
							td.appendChild(document.createTextNode(parseInt(rt)));
							tr.appendChild(td);
							
							tableForScatter.appendChild(tr);
							checkbox.addEventListener("click",toggleOnCheckBox(checkbox.id));
			...
			function rowHandlersForScatter() { 
					...
					for (i = 1; i < rows.length; i++) {
						...
						rows[i].onclick = function(){ return function(){
							...
							toggleResult(index,false);
					};}(rows[i]);
				}
			}
			function toggleOnCheckBox(index){
				console.log("index sent from click "+index);
				document.getElementById(index).checked=document.getElementById(index).checked==true? false:true;
							toggleResult(index.substring(index.length-1),true);
			}
			function toggleResult(index,flag){
				console.log("index sent from here "+index);
				//document.getElementById("chkID"+index).checked=document.getElementById("chkID"+index).checked==true? false:true;
			...
			function resetInPath(){
				document.getElementById("similarityRank").checked=false;
				document.getElementById("intensityRank").checked=false;
				clearMap();
			}
